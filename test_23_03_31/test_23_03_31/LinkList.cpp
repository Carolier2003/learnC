...
// 若插入失败，返回false，否则返回true。
{
    ...
    /********** Begin *********/
    // 创建一个新结点，数据域为x
    LinkNode* newNode = (LinkNode*)malloc(sizeof(LinkNode)); // 为新结点分配内存空间
    if (newNode == NULL) return false; // 如果分配失败，返回false
    newNode->data = x; // 将x赋值给新结点的数据域
    if (llist->len == 0) /*在空表中插入*/
    {
        newNode->next = NULL; // 新结点的后继指针指向NULL
        llist->front = llist->rear = newNode; // 头结点和尾结点都指向新结点
    }
    else if (llist->curr == llist->rear || llist->curr == NULL) /*在尾结点处插入*/
    {
        newNode->next = llist->curr->next; // 新结点的后继指针指向NULL
        llist->curr->next = newNode; // 原来的尾结点的后继指针指向新结点
        llist->pre = llist->curr; // 前驱指针指向原来的尾结点
        llist->position++; // 位置加一
    }
    else /*在链表的中间位置插入*/
    {
        newNode->next = llist->curr->next; // 新结点的后继指针指向原来当前位置之后的结点
        llist->curr->next = newNode; // 原来当前位置的后继指针指向新结点
        llist->pre = llist->curr; // 前驱指针指向原来的当前位置
        llist->position++; // 位置加一
    }
    /*增加链表的大小*/
    llist->len++;
